...
public class ShapeVoxelizer : MonoBehaviour
{
    [Header("Input Mesh")]
...
//}
    ...
    void AutoSetParameters(Vector3[] meshVertices)
    {
        float totalDistance = 0f;
        int count = 0;

        // Sample distances between neighboring vertices in triangles
        for (int i = 0; i < triangles.Length; i += 3)
        {
            Vector3 v0 = meshVertices[triangles[i]];
            Vector3 v1 = meshVertices[triangles[i + 1]];
            Vector3 v2 = meshVertices[triangles[i + 2]];

            totalDistance += Vector3.Distance(v0, v1);
            totalDistance += Vector3.Distance(v1, v2);
            totalDistance += Vector3.Distance(v2, v0);
            count += 3;
        }
        ...
        if (meshVertices.Length < 10) {
            maxEdgeLength = 0.1f;       // Allow modest subdivision
            vertexMergeThreshold = .025f; // Allow mild simplification
            }
        else                    // Set adaptive thresholds
        {
            maxEdgeLength = 2f;       // Allow modest subdivision
            vertexMergeThreshold = .05f; // Allow mild simplification
            }
    }
