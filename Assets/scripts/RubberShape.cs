...
public class RubberShape : MeshType
{
    ...
    public Vector3[] W;
    //public override CollisionDet Collision(CollisionDet col, SimulationState s)
    //{
    //    // ?? ????? ????? ??? ?????? ?? ???? ????????
    //    col.depth = -1f;
    ...
    public override void SimulatePhysics(SimulationState s, Transform transform, MeshObject1 self, MeshObject1[] allObjects)
    {
        ...
        foreach (var other in allObjects)
        {
            ...
            for (int i = 0; i < s.vertices.Length; i++)
            {
                ...
                {
                    // === POSITION CORRECTION ===
                    float beta = 1f; // ثبات اصطناعي
                    Vector3 correction = -normal * (beta * depth * Time.fixedDeltaTime);
                    ...
                    if (tangent.magnitude > 0.001f)
                    {
                        ...
                        s.velocities[i] += frictionImpulse / mA * .98f;
                        other.state.velocities[jIndex] -= frictionImpulse / mB * .98f;
                    }
